...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "S8CdY5M7mi7kEY7zH9w9xS9dzwkPQ3Fe";
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			keyword = URLEncoder.encode(keyword, "UTF-8"); // "Y Chen" => "Y%20Chen"
	...
	private List<Item> getItemList(JSONArray events) throws JSONException {
		...
		for (int i = 0; i < events.length(); ++i) {
			...
			//if (i == 15) {System.out.println(event.toString());}
	...
	private String getDateTime(JSONObject event) throws JSONException {
		if(!event.isNull("dates")) {
			...
			if(!dates.isNull("start")) {
				...
				if(!start.isNull("localDate")) {
					sb.append(start.getString("localDate"));
				}
				if(!start.isNull("localTime")) {
					sb.append(" | ");
					sb.append(start.getString("localTime"), 0, 5);
				}
				return sb.toString();
			}		
		}
